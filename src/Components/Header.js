...
import { addCache } from "../utils/slices/cacheSlice";

const Header = () => {
  const cachHistory = useSelector((state) => state.searchHistory);
  ...
  // fetching data to suggest user
  const handleSearch = async (query) => {
    try {
      if (!cachHistory[query]) {
        if (search) {
          const fetching = await fetch(SEARCH_LIST_API + query);
          const data = await fetching.json();
          setSuggestion(data[1]);
          dispatch(addCache({ [search]: data[1] }));
        }
      } else {
        setSuggestion(cachHistory[query]);
      }
    } catch (error) {
  ...
  return (
    <div className="grid grid-flow-col w-screen py-3 px-4 shadow-lg fixed backdrop-blur-2xl bg-white/80 dark:bg-black/80">
      <div className="flex gap-4 col-span-1 items-center">
        <IoMenu className="text-3xl cursor-pointer" onClick={handleToggle} />
        <Link to={"/"}>
          <div
            style={{
              width: "100%",
      ...
      <div className="flex col-span-10 items-center relative ">
        <input
          ...
          value={search}
          className="flex-1 border h-12 border-gray-400 py-2 px-4 outline-none rounded-l-full bg-transparent "
          type="text"
          placeholder="Search"
        />

        {isFocus && search.length > 0 && (
          <div
            className={`absolute mt-2 top-full bg-white dark:bg-black rounded-lg py-2 px-4 w-11/12  flex-col gap-2 flex`}
          >
            {suggestion?.map((suggest, idx) => (
              <div
                key={idx}
                className="hover:bg-white/20 cursor-pointer p-2 flex gap-3 items-center"
                onClick={() => gotoSearchResultPage(suggest)}
              >
                <span>
                  <CiSearch />
                </span>
        ...
        <div className="px-6 h-12 bg-gray-400 rounded-r-full flex items-center">
          <img
            onClick={() => gotoSearchResultPage(search)}
            className="h-8"
            alt="search-icon"
